...
def find_pairs(ctc_root: Path, seq: str, gt_subdir: str | None = None):
    img_dir = ctc_root / seq
    seg_dir = (ctc_root / gt_subdir) if gt_subdir else (ctc_root / f"{seq}_GT" / "SEG")
    ...
    if not common:
        raise FileNotFoundError(f"No matching frame indices between {img_dir} and {seg_dir}")
...
def list_frames(ctc_root: Path, seq: str):
    img_dir = ctc_root / seq
    frames = sorted(
        list(img_dir.glob("*.tif")) + list(img_dir.glob("*.tiff")),
